//              ***Error handling
// Error types: 
// 1. recoverable: program may continue with some unwanted side-effect. We can recover 
// from an erroneous state by using the Option enum (see before) and the Result enum (see in this lesson)
// 2. non-recoverable: failure, program shuts down. Also called panicking.

//              ***Throwing errors
// non-recoverable
// panic! stops execution of the program and displays its argument as an error message and 
// optionally a backtrace (in other languages: stack trace).
// RUST_BACKTRACE=1 cargo run: set backtrace to 1 to see the trace information.
// panic! should hardly ever be used in Rust programs, and should only be used for non-recoverable errors.
// ************************************************************************
// fn erroneous_call() {
...
// 1. to represent an optional value that either exists or not
// 2. a key does not unlock a value in a data structure (e.g. struct or vector)
// Generally, if you are tempted to use null or undefined in JavaScript, the Rust solution is the Option enum.
// ************************************************************************
...
// }
// ************************************************************************
// The option type can be unwrapped using the unwrap() method called on an Option. If the 
// optional value exists, unwrap() returns the value. If the value does not exist, the program panics.
// ************************************************************************
// fn print_value(v: &Vec<i32>, i: usize) -> () {
...
// The ? operator can be applied on an Option. If there is a value, the ? operator gives you the 
// same result as calling unwrap(). However, if there is no value, the function returns None. 
// As a result, when using ? on an Option in a function, the program has to return an Option.
// ************************************************************************
...
// A Result can be transformed to ValueType or panic using the unwrap() method of Result 
// instances:
// ************************************************************************
// fn divide(a: f32, b: f32) -> Result<f32, String> {
//   if b == 0.0 {
